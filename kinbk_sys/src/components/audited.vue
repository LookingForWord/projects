<template>
  <el-card class="box-card audited-card" v-loading.body="loading">
    <define-content :cgxpageHandle='cgxpageHandle' :tableData='tableData' :total="total" :pageUrl="pageUrl"
      ...
      >
        <template scope="scope">
          <router-link class='auditNoPass' v-if='scope.row.auditStatus == "未通过"'
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      if (this.$constant.AUDITEDURL) {
        let me = this;
        this.loading = true;
        this.total = 0;
        this.pageUrl = this.$constant.AUDITEDURL;
        this.$http({
          url: this.$constant.AUDITEDURL,
          vm: me,
          method: "GET",
