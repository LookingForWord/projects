<template>
  <el-card class="box-card audit-all-card" v-loading.body="loading">
    <define-content :cgxpageHandle='cgxpageHandle' :tableData='tableData' :total="total" :pageUrl="pageUrl"
      ...
      >
        <template scope="scope">
          <router-link class='unaudited' v-if='scope.row.auditStatus == "未审核"'
                       :to='{name: "auditDetail",params:{id:scope.row.id,mcId:scope.row.mcId}}'>{{scope.row.auditStatus}}<i
            class="el-icon-arrow-right"></i></router-link>
          <router-link class='auditNoPass' v-else-if='scope.row.auditStatus == "未通过"'
                       :to='{name: "auditNoPass",params:{id:scope.row.id,mcId:scope.row.mcId}}'>
            {{scope.row.auditStatus}}
          </router-link>
          <router-link class='auditPass' v-else :to='{name: "auditPass",params:{id:scope.row.id,mcId:scope.row.mcId}}'>
            {{scope.row.auditStatus}}
          </router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      if (this.$constant.AUDITALLURL) {
        let me = this;
        this.loading = true;
        this.total = 0;
        this.pageUrl = this.$constant.AUDITALLURL;
        this.$http({
          url: this.$constant.AUDITALLURL,
          vm: me,
          method: "GET",
...
<style lang='scss'>
  ...
  .audit-all-card {
    ...
    div.cell {
      ...
      width: 100% !important;
      box-sizing: border-box;
      a.unaudited, a.auditNoPass, a.auditPass {
