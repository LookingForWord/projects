<template>
  <el-card class="box-card refunded-card" v-loading.body="loading">
    <define-content :cgxpageHandle='cgxpageHandle' :tableData='tableData' :total="total" :pageUrl="pageUrl"
      ...
      >
        <template scope='scope'>
          <router-link class='refunded-detail'
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      if (this.$constant.REFUNDED) {
        let me = this;
        this.loading = true;
        this.total = 0;
        this.pageUrl = this.$constant.REFUNDED;
        this.$http({
          url: this.$constant.REFUNDED,
          vm: me,
          method: "GET",
