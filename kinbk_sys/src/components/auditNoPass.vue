<template>
  <el-card class="box-card audit-no-pass-card" v-loading.body="loading">
    ...
    <div class='user-msg'>
      <el-card class="box-card user-msg-header">
        ...
        <div class='audit-no-pass-detail'>
          审核未通过
        </div>
      </el-card>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      if (this.$constant.AUDITNOPASSDETAIL && id !== undefined && mcId !== undefined) {
        let me = this;
        this.loading = true;
        this.$http({
          url: this.$constant.AUDITNOPASSDETAIL,
          vm: me,
          method: "GET",
...
<style lang='scss'>
  .audit-no-pass-card {
    ...
    .audit-no-pass-detail {
      ...
      font-weight: bold;
      color: #FF8400;
      background-image: linear-gradient(to right, #ccc, #44e);
    }
    .users-form {
      .users-form-line {
        .el-form-item {
